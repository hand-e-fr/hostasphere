...
class Session:
    ...
    def record_usage(self):
        ...
        self.metrics.memory_usage.append(
            session_pb2.UsageAtTime(time=current_time,
    ...
    def save_metrics(self):
        try:
            while not self._stop_event.is_set():
                self.record_usage()
                sleep(
                    self._refresh_interval)  # Simulate time interval for metrics recording
    ...
    def end_session(self):
        ...
        self.metrics.execution_time = (
                                                  self.metrics.end_time - self.metrics.start_time) * 1000  # milliseconds
        self.save_session()

    def save_session(self):
        with grpc.insecure_channel(self._address) as channel:
            stub = session_pb2_grpc.SessionServiceStub(channel)
            request = session_pb2.SaveSessionRequest(token=self._token,
